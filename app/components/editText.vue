<template>
  ...
  <span :id="Id + '_component'" class=" divi inputDivi" :title="This.prop.ToolTipText" :style="Styles.style"
    ...
    <!--v-on:keyup.63="clickHelp()" v-on:keyup.13="keyDown"   currentValue[focusIn]
...
const maskOptions = reactive({
  mask: Number,
  scale: This.prop.Decimals,         // digits after point
  signed: +(This.prop.Min) < 0 ? true : false,            // allow negative numbers
  thousandsSeparator: ',',  // thousands separator
  padFractionalZeros: true, // pad fractional zeros with zeroes
...
const currencyMask = createNumberMask({
  ...
  decimalLimit: This.prop.Decimals,      // Máximo 4 dígitos después del punto (como en .9999)
  includeThousandsSeparator: true, // Incluye las comas
  thousandsSeparatorSymbol: ',',   // Separador de miles
  decimalSymbol: This.prop.Decimals > 0 ? '.' : '',              // Separador decimal
  allowNegative: false,            // Solo números positivos
  integerLimit: This.prop.IntegerLimit,   // Límite de dígitos enteros (opcional, ej. 999,999)
  prefix: This.prop.Currency, // Símbolo de moneda
...
const emitValue = async (readCam?: boolean, isValid?: boolean, newValor?: string) => {
  ...
  if (!readCam) { // En valor viene el valor actual capturado
    ...
    if (!watchPropValue) { // Solo se cambia a proceso si no se cambio el valor desde el watch This.prop.Value
      This.prop.Status = 'P'
  ...
  /* 18/Feb/2026 se prueba sw_emitValue=false
    if (!This.prop.ReadOnly && !This.prop.Disabled)
      sw_emitValue = true
    else
      sw_emitValue = false
  */
  //No quitarlo emite un update en el componente padre
  emit("update:Value", Value.value); // actualiza el valor Value en el componente padre

  if (This.onChangeValue) { // emite el evento cuando cambia de valor
...
// 1. Se ejecuta cuando el input se inicializa. Aquí asignamos el valor inicial.
const onInit = (e) => {

  console.log('onInit', e)
  maskInstance.value = e.detail;
  if (props.prop.Value !== null && props.prop.Value !== undefined) {
    maskInstance.value.typedValue = props.prop.Value;
  }
};

const onAccept = (e: any) => {
  const maskRef = e.detail;
  // Guardamos la instancia de IMask si no la teníamos mapeada
  if (!maskInstance.value) {
    maskInstance.value = maskRef;

    // Si el padre envió un valor inicial válido, se lo inyectamos a la fuerza
    if (Value.value !== null && Value.value !== undefined) {
      maskInstance.value.typedValue = Value.value
    }
  }

  if (Number.isNaN(Value.value) || Value.value == null) {
    Value.value = 0
  }

  // console.log('Fin ) onAccept  maskInstance.value', maskInstance.value, 'Value', Value.value)

  // Notificamos el número limpio (Number) de vuelta al componente padre
  //  emit('update:modelValue', maskRef.typedValue);
};
...
// Params: Si se utiliza un parametro, el parametro es el objeto html que lo emitio
/////////////////////////////////////////////////////////////////
const onFocus = async () => {
  ...
  */
  ...
  if (This.Parent.prop.BaseClass == "grid")
    This.Parent.Column = This.prop.Name

  focusIn.value = 1  // cambia el valor en el input number 
  console.log('7-fin) Fin ======>editText onFocus Name=', This.prop.Name, 'This.Parent.Column=', This.Parent.Column)
}
...
onMounted(async (e) => {
  ...
  console.log('editText mounted propType.value=', propType.value)
  if (propType.value == 'number') {
    ...
    // onInput(This.prop.Value.toString())
    console.log('FormatValue', maskInstance.value, 'OriginalValue', This.prop.Value, This.prop.Min, This.prop.Max)
    /* 27/May/2026
...
onUnmounted(async () => {
  if (This.onUnmounted) await This.onUnmounted()
})
