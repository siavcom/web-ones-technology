...
export class ThisForm extends reportCXC {
  ...
  public async sqlQuery(where: string, order: string) {
    ...
    if (cx_tcd_tcd.trim() != "??") {
      ...
      const data = await this.Sql.localAlaSql(` select des_tcd from now.loc_cometcd where tdo_tdo='${this.cx_tdo_tdo.prop.Value}' and tcd_tcd='${this.cx_tcd_tcd.prop.Value}'  `)
...
`
    con_vis = con_vis + ` and ${doc_imp} AND (fac.MON_DOC=${mon_rep}
           ) AND fac.FEC_DOC<='${has_fec}' and (imp_doc+im1_doc+im2_doc+im3_doc-${pag_tot}> .09 or 
                           ( tdo_fac is not  null ))     
 `
    ...
    select fac.tdo_tdo as tdo_tdo,tfa.des_tdo as des_fac,ndo_doc,fac.cod_nom,nom.nom_nom,ref_doc,(imp_doc+im1_doc+im2_doc+im3_doc) as imp_fac,
