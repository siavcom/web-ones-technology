...
export class ThisForm extends reportInv {
  ...
  constructor() {
    ...
    this.for_imp.prop.Value = 'jr_come3210'   // no incluir extencion jasper o jrxml
  ...
  public override async init() {
    ...
    if (par_for == "IN")
      ...
      this.in_tdo_tdo.prop.RowSource =
        ` select des_tdo,tdo_tdo,inv_tdo from now.loc_cometdo where cop_nom='${par_for}' union select '  Todos ' as des_tdo,'?? ' as tdo_tdo,'N' as inv_tdo order by des_tdo `
  ...
  public async sqlQuery(where: string, order: string) {
    ...
    if (this.Form.Params[0] == "´IN´")
      con_vis = `( (fec_mov>=${des_fec_que} AND fec_mov<=${has_fec_que}) and can_mov<>0  ${doc_imp} ${alm_sql} ) `
    else
      con_vis = `( (FEC_DOC>=${des_fec_que} AND FEC_DOC<=${has_fec_que}) and can_mov<>0  ${doc_imp} ${alm_sql} ) `
