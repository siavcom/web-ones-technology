...
// Class : reportInv
// Description : Existencia por pedimento
// Author : MGSR
// Creation : 2025/02/21
// Update Date  : 
/////////////////////////////////////////////
...
export class ThisForm extends reportInv {
  ...
  constructor() {
    ...
    this.prop.Name = 'come3212'
    this.prop.Caption = "Existencias por pedimento (come3212)"
    this.prop.Status = 'A'
    this.prop.Development = true
    this.vis_rep = 'vi_come3212'   // nombre de la vista sql a utilizar en el reporte
    this.for_imp.prop.Value = 'jr_come3212'   // no incluir extencion jasper o jrxml
  ...
  public async sqlQuery(where: string, order: string) {
    ...
    var ins_sql = ' ',
      sql_alm = 'null'
    tit_rep = ' Todos los almacenes '

    if (alm_rep.trim() != '??') {
      if (localWhere.length > 3)
        localWhere = localWhere + ' and '
      localWhere = localWhere + ` alm_tda='${alm_rep}' `
      sql_alm = "'" + alm_rep + "'"
      tit_rep = ' Almacen ' + alm_rep
    }
    this.Form.tit_rep = tit_rep
    if (localWhere.length > 3)
      localWhere = localWhere + ' and '
    var has_fec_exi = "'" + has_fec.replaceAll('-', '') + "'"
    localWhere =
      localWhere +
      `   fec_mov<='${has_fec}'  ` + where;
    ins_sql = ` if not( OBJECT_ID('tempdb..#resultado') is null) drop table #resultado 
         if not( OBJECT_ID('tempdb..#exist') is null) drop table #exist ;
      SELECT cla_isu as cla_con,ped_ped,fec_ped,adu_adu,age_ped,
      SUM(case when ${this.vis_rep}.INV_TDO='E' then CAN_MOV else 0 end ) AS can_ent, 
	    ...
	    where ${localWhere}
	    GROUP by cla_isu,ped_ped,fec_ped,adu_adu,age_ped 
      select distinct cla_con cla_exi,
      dbo.F_CAL_EXI_GEN(cla_con,0,${has_fec_exi},${sql_alm},NULL,NULL,NULL)
       as exi_tot into #exist from #RESULTADO  
        SELECT *,${que_fam} as des_fam FROM #RESULTADO JOIN vi_come3201 vcome3201 ON #RESULTADO.CLA_CON=vcome3201.CLA_ISU 
        join #exist on cla_exi=#resultado.cla_con 
	      ${order} ; drop table #exist ; drop table #resultado 
