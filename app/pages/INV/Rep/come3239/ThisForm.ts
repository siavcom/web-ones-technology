...
export class ThisForm extends reportInv {
  ...
  constructor() {
    ...
    this.prop.Name = 'come3239'
    this.prop.Caption = "Valorización del inventario (costo promedio de solo compras)"
    this.prop.Status = 'A'
    this.prop.Development = true
    this.vis_rep = 'vi_come3207'   // nombre de la vista sql a utilizar en el reporte
    this.for_imp.prop.Value = 'jr_come3239'   // no incluir extencion jasper o jrxml
    ...
    this.des_fec.prop.Caption = 'Calculo de costo desde la fecha '
  ...
  public async sqlQuery(where: string, order: string) {
    ...
    const des_fec = dateToSql(this.Form.des_fec.prop.Value); //dateToSql(this.Form.has_fec.Value)
    ...
    fec_exi_q = await addDay(fec_exi_q, 28);
    ...
    var tit_rep = ''
    if (this.alm_rep.prop.Value.trim() != '??') {
    ...
    localWhere = `  exec p_cal_cos_pro_deh '','ZZZZZZZZZZZZZZZZZ','1'; 
        ...
        dbo.f_cal_cos_com(cla_isu,'${des_fec}','${has_fec}') as cos_com,
         *,'${fec_exi_q}' as fec_exi,${que_fam} as des_fam from  ${this.vis_rep} where ${localWhere}  ${order}; `
