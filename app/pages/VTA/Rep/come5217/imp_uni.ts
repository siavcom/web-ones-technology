//////////////////////////////////////////////
// this Form was generated automatically for web-ones-technology
// BaseClass : OPTIONGROUP
// Class : imp_uni
// Description : reporte en importes o unidades
// Author : MGSR
// Creation :2025-07-07
// Update Date  :
/////////////////////////////////////////////
// import {optionGroup} from "@/classes/optionGroup";

import { OPTIONGROUP } from "@/classes/OptionGroup";
import { option } from "~/classes/Option";
//imports

export class imp_uni extends OPTIONGROUP {
  //public

  constructor() {
    super();
    this.prop.BaseClass = "optionGroup";
    this.prop.Name = "imp_uni";
    this.prop.Caption = "Reporte en ";
    // this.Option1.prop.Caption = "No";
    // this.Option1.position.Left = 5;
    this.Option1.prop.Caption = "Importes";
    this.Option2.prop.Caption = "Unidades";
    this.prop.Visible = true;
    this.prop.Value = 2;
    this.style.width = '295px'
    //propiedades
  }


  Option1 = new class extends option {
    constructor() {
      super(1);

    }
  }   // Fin Procedure

  Option2 = new class extends option {
    constructor() {
      super(2);
    }
  }

}
