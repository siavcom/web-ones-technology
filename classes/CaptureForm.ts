...
export class captureForm extends FORM {
  ...
  async validComponent(compName?: string) {
    ...
    if (data.length == 0) {
     ...
     /*
      console.log(
        "captureForm appendBlank  datos=",
        await this.Form.db.localAlaSql(
          "select * from " + this.prop.RecordSource
        )
      );
     */
    ...
    for (const i in this.main) {
      ...
      if (this[comp].prop.Capture) {
        if (this[comp].prop.updateKey === false) {
          ...
          if (Recno > 0) this[comp].Recno = Recno; // Actualiza el registro del componente
