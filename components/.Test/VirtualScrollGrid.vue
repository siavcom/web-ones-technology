<template>
  <div v-if="props.prop.Visible && props.prop.RecordSource.length > 0 && props.db.value && Db.View[prop.RecordSource]"
    ...
    <Grid :length="10" :pageSize="8" :scrollTo="10" :pageProvider="loadData" class="grid">
...
const loadData = async (pageNumber: number, pageSize: number) => {
  console.log('Load Data Page and size ===>>', pageNumber, pageSize)
  const result = []
  const Rows = pageSize ? props.prop.Rows : 10
  for (let i = 0; Rows - 1; i++) {
    const elementNo = ((pageNumber - 1) * Rows) + i
