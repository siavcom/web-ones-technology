...
export class tzo_usr extends CAPTURECOMPONENT {
   ...
   constructor() {
      ...
      let i = 0
      for (const item of tzo) {
         this.prop.RowSource[0][i] = item.TimeZone
         this.prop.RowSource[1][i] = item.UtcOffset
         this.prop.RowSource[2][i] = item.name
         i++
      }
   }

   override async when() {
      const data = await this.Sql.scatter('FIELDS', ['key_pri'], 'vi_cap_db_users')
      if (data.key_pri == 0) {
