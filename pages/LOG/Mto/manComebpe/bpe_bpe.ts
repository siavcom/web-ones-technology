...
export class bpe_bpe extends captureComponent {
  constructor() {
    ...
    this.prop.RowSource = `select ' Nueva boleta '+cast(max(bpe_bpe)+1 as char(7)) as boleta, max(bpe_bpe)+1 as bpe_bpe,' ' as cam_bpe  from man_comebpe where bas_bpe=1`;

    this.prop.ColumnCount = 3;
    this.prop.BoundColumn = 2;
    this.prop.ColumnWidths = "40%,10%,40%";
  ...
  async when() {
    /*
     this.prop.RowSource = `select cast(' Nueva boleta '+cast(max(bpe_bpe)+1 as char(7)) as char(20)) as boleta, max(bpe_bpe)+1 as bpe_bpe,'' as cam_bpe  from man_comebpe where bas_bpe=${this.Form.bas_bpe.prop.Value} union \
    select 'Boleta             ' as boleta ,bpe_bpe, \
    case \
       when tip_bpe=1 \ 
          then cast(pla_bpe as char(20)) \ 
       when tip_bpe=0 \
         then cast(cam_cam as char) end as cam_bpe from man_comebpe \
          where bas_bpe=${this.Form.bas_bpe.prop.Value} and pe2_bpe=0 order by boleta`;
  ...
  */
    ...
    this.prop.RowSource = `select cast(' Nueva boleta ' as char(15)) as boleta, max(bpe_bpe)+1 as bpe_bpe,'' as cam_bpe  from man_comebpe where bas_bpe=${this.Form.bas_bpe.prop.Value} union \
    select 'Boleta             ' as boleta ,bpe_bpe, \
    case \
       when tip_bpe=1 \ 
          then cast(pla_bpe as char(15)) \ 
       when tip_bpe=0 \
         then cast(cam_cam as char) end as cam_bpe from man_comebpe \
          where bas_bpe=${this.Form.bas_bpe.prop.Value} and pe2_bpe=0 order by boleta`;
