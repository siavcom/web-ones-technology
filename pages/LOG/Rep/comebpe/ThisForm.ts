...
///////////////////////////////////////////////////////////////
  ...
  public async sqlQuery(where: string,order: string) {
    ...
    if (tip_bpe!= 9)
       { if (localWhere.length>0)
          localWhere = localWhere + ` and tip_bpe='${tip_bpe}' `
           else 
           localWhere = localWhere + ` tip_bpe='${tip_bpe}' `
       }
    if (bas_bpe!= 9)
    { if (localWhere.length>0)
      localWhere = localWhere + ` and bas_bpe=${bas_bpe} `
       else 
       localWhere = localWhere + ` bas_bpe=${bas_bpe} `
   }
  ...
  if (localWhere.length>0)
    ...
    localWhere=localWhere+`  tp1_bpe>='${des_fec}' AND tp1_bpe<='${has_fec}' `+where;
