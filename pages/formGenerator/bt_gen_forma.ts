...
export class bt_gen_forma extends COMPONENT {
  ...
  async generaForma(zipWriter: any) {
    ...
    if (
      ...
      ThisForm = ThisForm.replaceAll(
        "<<vis_cap>>",
        this.Form.vis_form.prop.Value.trim()
      );
    ...
    while (this.Form.tip_for.prop.Value.length > 0) {
      ...
      for (let i = 0; i < renglon.length; i++) {
        ...
        imp_com =
          imp_com +
          `import {${cam_dat.trim()} } from "./${cam_dat.trim()}" ` +
          String.fromCharCode(10);

        com_imp =
          com_imp +
          `   public ${cam_dat.trim()} = new ${cam_dat.trim()}() ` +
          String.fromCharCode(10);
        ...
        if (tip_dat == "int" && lon_dat == "1") Type = "checkBox";
        if (
          tip_dat == "var" ||
          ((tip_dat == "cha" || tip_dat == "var") && lon_dat > 256)
        ) {
          Type = "textArea";
          MaxLength = lon_dat;
        }
        ...
        else row_com = row_com.replaceAll("<<Capture>>", "false");

        //genera cada clase de c componente

        // Generamos archivo de clases
        console.log(
          "bt_gen_forma renglon",
          `${nom_for}/${grid}${cam_dat.trim()}.ts`
        );
        await zipWriter.add(
          `${nom_for}/${grid}${cam_dat.trim()}.ts`,
          new TextReader(row_com)
        );
      }
      // ) // Final for each
      let init = "";
      ...
      if (vis_grid == "vi_cap_grid") {
        ...
        await zipWriter.add(
          `${nom_for}/grid/grid.ts`,
          new TextReader(ThisGrid)
        );
        com_imp = "";
        imp_com = "";
        ...
        if (this.Form.tip_for.prop.Value == "C") {
          ...
          this.Form.tip_for.prop.Value = "F";
          grid = "";
        } else this.Form.tip_for.prop.Value = "";
      } else this.Form.tip_for.prop.Value = "";
